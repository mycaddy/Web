...
mock.onGet('/api/club-app/clubs').reply((config) => {
  const params = config.params;
  let response = [];
  if (params.labelHandle) {
    const labelId = clubDB.labels.find(label => label.handle === params.labelHandle).id;

    response = clubDB.clubs.filter((club) => club.labels.includes(labelId) && !club.deleted);
  }
  else if (params.filterHandle) {
    if (params.filterHandle === 'deleted') {
      response = clubDB.clubs.filter((club) => club.deleted);
    }
    else {
      response = clubDB.clubs.filter((club) => club[params.filterHandle] && !club.deleted);
    }
  }
  else // folderHandle
  {
    ...
    if (folderHandle === 'all') {
      ...
      const folderId = clubDB.folders.find(folder => folder.handle === folderHandle).id;
      response = clubDB.clubs.filter((club) => club.folder === folderId && !club.deleted);
    }
  }
...
mock.onPost('/api/club-app/update-club').reply((request) => {
  const club = JSON.parse(request.data);

  clubDB.clubs = clubDB.clubs.map((_club) => {
    if (_club.id === club.id) {
      return club;
    }
    return _club;
...
mock.onPost('/api/club-app/new-club').reply((request) => {
  const club = JSON.parse(request.data);
...
mock.onPost('/api/club-app/remove-club').reply((request) => {
...
mock.onGet('/api/club-app/filters').reply(200, clubDB.filters);
mock.onGet('/api/club-app/labels').reply(200, clubDB.labels);
mock.onGet('/api/club-app/folders').reply(200, clubDB.folders);


mock.onPost('/api/club-app/set-folder').reply((request) => {
  const data = JSON.parse(request.data);
  const { selectedClubIds, folderId } = data;
  clubDB.clubs = clubDB.clubs.map((_club) => {

    if (selectedClubIds.includes(_club.id)) {
      return {
        ..._club,
        folder: folderId
      };
...
mock.onPost('/api/club-app/toggle-label').reply((request) => {
  const data = JSON.parse(request.data);
  const { selectedClubIds, labelId } = data;
  clubDB.clubs = clubDB.clubs.map((_club) => {
    if (selectedClubIds.includes(_club.id)) {
      return {
        ..._club,
        labels: _club.labels.includes(labelId) ? _club.labels.filter(_id => _id !== labelId) : [..._club.labels, labelId]
      };
    }
    return _club;
  });
...
mock.onPost('/api/club-app/delete-clubs').reply((request) => {
  const data = JSON.parse(request.data);
  const { selectedClubIds } = data;
  clubDB.clubs = clubDB.clubs.filter((_club) => selectedClubIds.includes(_club.id) ? false : _club);
  return [200];
});
